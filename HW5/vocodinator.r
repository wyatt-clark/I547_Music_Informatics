...
remove_every_other = function(y){
	
	y_temp = seq(0,floor(length(y)/2));
	for(t in 1:length(y_temp)){
		y_temp[t] = y[t*2];	
	}
	y = y_temp;
	print("take it apart")
...
#first method!!!!!!!   creates a bad sound
#######################
#######################
#in order to increase the frequency I need to 
#shift my energies at frequency i to frequency 2*i.  this requires 
#going through the mod of the comples number and shifting each energy up 2*i
...
#We want to remove every other sample, then double each frame in the stft
#y = remove_every_other(y);
#Y = stft(y,H,N)
...
#######################
#######################
# create a stft that goes twice as fast but MAINTAINS THE PITCH
#Ybar = matrix(0,nrow(Y),ncol(Y)/2)  
#for (t in 1:(ncol(Y)/2)) { 
#	  Ybar[,t] = Y[,2*t] 
#}
#######################
...
#Ybar = matrix(0,nrow(Y),ncol(Y)/2)  # create a stft that goes twice as fast but maintains the pitch
#for (t in 1:(ncol(Y)/2)) { 
#  Ybar[,t] = Y[,2*t] 
#}
